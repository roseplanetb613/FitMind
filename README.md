...
**主界面** —— 28 块肌群按恢复度着色（冷 = 恢复好，暖 = 需休息），
左下是恢复度图例，左上工具切换正 / 背面、计划抽屉与显示开关，
右侧是常驻的对话面板：

![主界面：3D 肌群恢复视图与对话教练](assets/screenshots/01-overview.png)

**打开标签** —— 每块肌群显示中文名与恢复百分比。这些措辞与数字全部由后端算好
（`pct` / `describe`），前端只做呈现：

![肌群标签：中文名与恢复百分比](assets/screenshots/02-labels.png)

**对话** —— 问「我今天练了胸，明天能练背吗」，回答依据图谱里各肌群的真实恢复状态；
回复下方的动作卡带示例图、器械与难度中文名，卡片底部是媒体署名：
