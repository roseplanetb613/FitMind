...
# ⚠ **键集必须与 _INJURY_SITE 完全一致**（2026-09-13 修）：`extract_injury_site`
# 遍历的是 _INJURY_SITE 的键、取的却是 _INJURY_SITE_ZH 的值，两表键集一漂，
# 扫描到那个键的句子就**抛 KeyError**。实测缺的正是「手肘」：
#     _INJURY_SITE 18 键（含手肘）  _INJURY_SITE_ZH 17 键（缺手肘）
#     "我手肘疼" → KeyError: '手肘' → 被 _memory_record 的 except: pass 吞掉
#     → guard 照回黄色警告、图谱里一条没记、diag 也没有计数（三层全静默）
# 注意「手肘」与「肘」都在表里是**有意的**（前者在扫描序里靠前，先把三字词吃住），
# 所以值映射到同一个 canonical「手肘」。一致性由 test_memory_guard 的键集用例守。
_INJURY_SITE_ZH = {
    "膝": "膝", "半月板": "膝", "十字韧带": "膝", "交叉韧带": "膝", "腰": "腰", "肩": "肩", "手腕": "手腕",
    "手肘": "手肘", "肘": "手肘",
