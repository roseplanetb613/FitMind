...
### 2.1 `per_100g`(12 项,对齐 foods_core)

`calories_kcal`/`protein_g`/`fat_g`/`carbs_g(CHO)`/`fiber_g(dietaryFiber)`/`sugar_g(**恒为 None,无糖细分**)/`sodium_mg(Na)`/`cholesterol_mg`/`vitamin_c_mg`/`calcium_mg(Ca)`/`iron_mg(Fe)`
