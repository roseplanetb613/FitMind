...
| `serving_g` | **配方总生重（克）**，恒等于 `recipe` 各项 `grams` 之和。**≠ 成菜重量**：只算生重，不含汤水与煎炸吸油——如兰州牛肉面「一碗」= 264g 干面+牛肉+油+香料，实际一碗连汤 500g+ |
...
**配方与别名表里的食用油，必须引用宏量完整（尤其 `calories_kcal` 非 `None`）的条目，
禁用中文表 `cn_1920xx`。**

注意**真正的约束是「宏量完整」，不是「必须 `fdc_*`」**——`fdc_*` 只是达成它的通常路径
（实践中绝大多数油都是这么解的）。唯一的例外是 `椰子油` → `off_645365c7`
（OpenFoodFacts，900 kcal、宏量完整）。判定这件事的是 `DishRepo._usable()`，
它按宏量判、不按来源判；把规则记成"必须 fdc_"会在下一个非 fdc 的完整条目上卡住。
