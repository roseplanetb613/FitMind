...
// tests/layout.test.ts 的包围盒断言防「放大 / 位置飞出人体」（全局 min/max）。
// 它抓不到「体块单纯缩小、仍留在包络内」——实测 forearms x 0.235→0.0235、
// cardio y 1.37→0.137 都照样通过。别把这条断言当成尺寸正确性的完备保证。
// 逐块自查另防「scale 非正」（≤ 0 与 NaN）——这是全局 min/max 覆盖不到的一项。
